...
exports.getDeptDetails = async (req, res) => {
  try {
    const result = await staffDAL.getDeptDetails();
    res.send(result);
  } catch (e) {
    res.status(500).send(e);
    throw e;
...
exports.submitDoc = async (req, res) => {
  try {
    ...
    staffDAL.addActivityLog(
      ...
      ip.address(),
      getURL(req),
      `${parser.setUA(req.headers["user-agent"]).getOS().name} ${parser.setUA(req.headers["user-agent"]).getOS().version
      }`,
      `${parser.setUA(req.headers["user-agent"]).getBrowser().name} ${parser.setUA(req.headers["user-agent"]).getBrowser().version
      }`
    );
    res.send(result);
  } catch (e) {
...
exports.submitDocmentdetails = async (req, res) => {
  try {
    ...
    upload(req, res, async err => {
      ...
      const result = await staffDAL.submitDocmentdetails(mdata,req.session.userID);
      // staffDAL.addActivityLog('/submitDocmentdetails', 'INSERT', 'POST', req.session.userID, ip.address(), getURL(req), req.device.type.toUpperCase(), `${parser.setUA(req.headers['user-agent']).getOS().name} ${parser.setUA(req.headers['user-agent']).getOS().version}`, `${parser.setUA(req.headers['user-agent']).getBrowser().name} ${parser.setUA(req.headers['user-agent']).getBrowser().version}`);
      res.send(result);
...
exports.getDocumentDetails = async (req, res) => {
  try {
    const result = await staffDAL.getDocumentDetails(req.session.userID);
    res.send(result);
  } catch (e) {
    res.status(500).send(e);
    throw e;
...
exports.requestData = async (req, res) => {
  try {
    const result = await staffDAL.requestData(req.body, req.session.userID);
    staffDAL.addActivityLog(
      "/submitDoc",
      "INSERT",
      "POST",
...
exports.submitReuploadDocmentdetails = async (req, res) => {
  try {
    const result = await staffDAL.submitReuploadDocmentdetails(
      req.body,
      req.session.userID
    );
    staffDAL.addActivityLog(
...
exports.getdeptidData = async (req, res) => {
  try {
    const result = await staffDAL.getdeptidData(req.session.userID);
    res.send(result);
  } catch (e) {
    res.status(500).send(e);
    throw e;
